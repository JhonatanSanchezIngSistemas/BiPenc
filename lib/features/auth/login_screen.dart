...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _postRegistroExitoso() async {
    ...
    try {
      ...
      await prefs.setBool('welcome_pending_${session.user.id}', true);
    } catch (e) {
      AppLogger.warning(
  ...
  Future<void> _ingresarOffline() async {
    ...
    setState(() => _isProcessingForm = true);
    final ok = await SecurityService().validarAccesoAdministrador(pin);
    setState(() => _isProcessingForm = false);
  ...
  Future<void> _ingresarApp() async {
    if (mounted) {
      context.read<SessionManager>().marcarSesionAutenticada();
      context.go('/pos');
    }
  }
