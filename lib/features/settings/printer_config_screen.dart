...
class _PrinterConfigScreenState extends State<PrinterConfigScreen> {
  ...
  Future<void> _probarImpresion() async {
    await _saveHeaders();
    final ok = await PrintService().imprimirPruebaCalibracion();
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
  ...
  Future<void> _selectPrinter(BluetoothInfo device) async {
    ...
    await PrintService().intentarReconectar();
        ...
        child: Column(
          ...
          children: [
            ...
            FutureBuilder<Map<String, dynamic>>(
              future: PrintQueue.obtenerEstadisticas(),
  ...
  void _previewTicket() {
    final dummyItem = ItemCarrito(
      ...
      presentacion: Presentation.unit,
      cantidad: 1,
    );
    final text = PrintService.renderTicketPreview(
      items: [dummyItem],
