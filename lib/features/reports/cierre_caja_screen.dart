...
class _CierreCajaScreenState extends State<CierreCajaScreen> {
  ...
  Future<void> _refresh() async {
    ...
    final resumen = await LocalDbService.obtenerResumenTurnoActual();
  ...
  Future<void> _abrirTurno() async {
    ...
    await LocalDbService.abrirTurnoCaja(montoApertura: monto, aliasUsuario: alias);
    await _refresh();
    if (!mounted) return;
  ...
  Future<void> _cerrarTurno() async {
    ...
    final alias = await SupabaseService.obtenerAliasVendedorActual();
    final closed = await LocalDbService.cerrarTurnoCaja(
      turnoId: _turnoId!,
            ...
            OutlinedButton.icon(
              onPressed: () async {
                ...
                try {
                  ...
                  await SupabaseService.procesarSyncQueue();
                  await SupabaseService.procesarSyncQueueV2();
                  final purged = await LocalDbService.purgarDatosAntiguos();
                  if (!mounted) return;
