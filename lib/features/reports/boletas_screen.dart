...
class _BoletasScreenState extends State<BoletasScreen> {
  ...
  Future<void> _cargar({String? query}) async {
    ...
    final rows = await LocalDbService.buscarVentasPorCorrelativo(
      query ?? _search.text,
      limit: 120,
    );
    if (!mounted) return;
  ...
  Future<Venta> _mapVenta(Map<String, dynamic> row) async {
    ...
    final subtotal = row['subtotal'] != null
        ? double.tryParse(row['subtotal'].toString()) ?? (total / (1 + ivaRate))
        : (total / (1 + ivaRate));
    ...
    return Venta(
      ...
      fecha: DateTime.tryParse((row['creado_at'] ?? '').toString()) ?? DateTime.now(),
      ...
      metodoPago: MetodoPago.values.firstWhere(
        (e) => e.name.toUpperCase() == (row['metodo_pago']?.toString().toUpperCase() ?? 'EFECTIVO'),
  ...
  Future<void> _reimprimir(Map<String, dynamic> row) async {
    if ((row['anulado'] ?? 0) == 1) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('La boleta está anulada. No se puede reimprimir como válida.'),
    ...
    final ok = await PrintService().imprimirComprobante(venta);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(ok ? 'Reimpresión enviada' : 'No se pudo imprimir ahora, quedó en cola'),
  ...
  Future<void> _exportarPdf(
    ...
    final venta = await _mapVenta(row);
    final vendedor = await PrintService.obtenerAliasVendedor();
    final File pdf = await PdfService.generateVentaPdf(
      items: venta.items,
      total: venta.total,
      vendedor: vendedor,
      correlativo: venta.correlativo?.isNotEmpty == true ? venta.correlativo! : venta.id,
  ...
  Future<void> _anularBoleta(Map<String, dynamic> row) async {
    ...
    final alias = await SupabaseService.obtenerAliasVendedorActual();
    final localOk = await LocalDbService.anularVentaLocal(
      ventaId: ventaId,
      motivo: motivo,
      aliasUsuario: alias,
    );
    ...
    if (correlativo != null && correlativo.isNotEmpty) {
      cloudOk = await SupabaseService.anularVentaEnNube(
        correlativo: correlativo,
        motivo: motivo,
        aliasUsuario: alias,
      );
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          cloudOk
              ? 'Boleta anulada y sincronizada'
              : 'Boleta anulada localmente (pendiente de sincronizar)',
        ),
  ...
  Widget build(BuildContext context) {
    ...
    final pendientesSync = _ventas.where((v) => (v['sincronizado'] ?? 0) != 1).length;
                  ...
                  : visibles.isEmpty
                      ...
                      : RefreshIndicator(
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (_, i) {
                              final v = visibles[i];
                              final total = ((v['total'] ?? 0) as num).toDouble();
                              final anulada = (v['anulado'] ?? 0) == 1;
                              final estadoSync = (v['sincronizado'] ?? 0) == 1;
                              final creadoAt = DateTime.tryParse((v['creado_at'] ?? '').toString());
                              ...
                              return Card(
                                ...
                                child: ListTile(
                                  title: Text(v['correlativo']?.toString() ?? 'Ticket #${v['id']}'),
                                  subtitle: Text(
                                  ...
                                  trailing: Wrap(
                                    ...
                                    children: [
                                      IconButton(
                                        ...
                                        onPressed: anulada ? null : () => _reimprimir(v),
                                      ),
                                      IconButton(
                                        tooltip: 'PDF',
                                        icon: const Icon(Icons.picture_as_pdf_outlined),
                                        onPressed: anulada ? null : () => _abrirOpcionesPdf(v),
                                      ),
                                      IconButton(
                                        tooltip: 'Anular',
                                        icon: const Icon(Icons.block_outlined),
                                        onPressed: anulada ? null : () => _anularBoleta(v),
                                      ),
                                    ],
                                  ),
                                ),
