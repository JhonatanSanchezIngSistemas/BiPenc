...
class PrintQueue {
  ...
  static Future<Database> _database() async =>
      databaseOverride != null ? databaseOverride!() : LocalDbService.database;
  ...
  static Future<void> encolarImpresion({
    ...
    try {
      ...
      AppLogger.info('Comprobante encolado: $ventaId (tipo: $tipoComprobante)',
          tag: 'PRINT_QUEUE');

      // Intenta imprimir AHORA, si falla queda en cola para después
      await procesarCola();
    } catch (e) {
      AppLogger.error('Error encolando impresión',
  ...
  static Future<void> _procesarColaInternal() async {
    try {
      ...
      AppLogger.info('Procesando ${pendientes.length} items de print_queue',
          tag: 'PRINT_QUEUE');
      ...
      for (final item in pendientes) {
        ...
        try {
          ...
          AppLogger.error('Excepción imprimiendo cola $ventaId: $e',
              tag: 'PRINT_QUEUE', error: e);
        }
      ...
      AppLogger.error('Error procesando print_queue',
          tag: 'PRINT_QUEUE', error: e);
    }
  }
  ...
  static Future<void> vaciar() async {
    try {
      ...
      AppLogger.warn('Cola de impresión vaciada ($deleted registros)',
          tag: 'PRINT_QUEUE');
    } catch (e) {
      AppLogger.error('Error vaciando print_queue',
          tag: 'PRINT_QUEUE', error: e);
    }
  }
  ...
  static Future<void> limpiarColaAntigua() async {
    try {
      ...
      AppLogger.error('Error limpiando print_queue',
          tag: 'PRINT_QUEUE', error: e);
    }
  }
  ...
  static Future<Map<String, dynamic>> obtenerEstadisticas() async {
    try {
      ...
      AppLogger.error('Error obteniendo estadísticas',
          tag: 'PRINT_QUEUE', error: e);
      return {
        'pendientes': 0,
        'impresos': 0,
