...
class Producto {
  ...
  final String stockStatus; // 'DISPONIBLE', 'BAJOSTOCK', 'AGOTADO'
  ...
  factory Producto.fromJson(Map<String, dynamic> json) {
    return Producto(
      ...
      stockStatus: json['stock_status'] ?? 'DISPONIBLE',
    );
  }
  ...
  double _precioPorTipo(String type) {
    ...
    if (type == 'WHOLESALE') {
      final presMayorista = presentaciones.firstWhere(
        (p) => p.id == IdsPresentacion.mayorista,
        orElse: () => presentaciones.first,
      );
      return presMayorista.getPriceByType('NORMAL');
    }
    if (type == 'SPECIAL') {
      final presEspecial = presentaciones.firstWhere(
        (p) => p.id == IdsPresentacion.especial,
        orElse: () => presentaciones.first,
      );
      return presEspecial.getPriceByType('NORMAL');
    }
...
class ItemCarrito {
  ...
  factory ItemCarrito.fromJson(Map<String, dynamic> json) {
    ...
    final stubPresentacion = Presentacion(
      id: json['presentacion_id'] ?? 'unid',
      skuCode: productoId,
      name: presentacionNombre.isEmpty ? 'Unitario' : presentacionNombre,
      conversionFactor: 1.0,
      prices: [
        PuntoPrecio(type: 'NORMAL', amount: (normalPrice as num).toDouble())
      ],
    );
  ...
  /// Factory from normalized SQLite table `venta_items`
  factory ItemCarrito.fromLocalMap(Map<String, dynamic> map) {
    final productoId = map['producto_id']?.toString() ?? '';
    final productoNombre = map['producto_nombre']?.toString() ?? 'Producto';
    final sku = map['sku']?.toString() ?? productoId;

    // Crear stub de producto para consistencia.
    // Nota: Si necesitamos marcas/categorías reales, deberíamos buscar el producto en cache.
    // Pero para PDF el nombre y el ID suelen bastar si la tabla está bien poblada.
    final stubProducto = Producto(
      id: productoId,
      skuCode: sku.isNotEmpty ? sku : productoId,
      nombre: productoNombre,
      presentaciones: [],
    );

    final stubPresentacion = Presentacion(
      id: map['presentacion_id']?.toString() ?? 'unid',
      skuCode: sku.isNotEmpty ? sku : productoId,
      name: map['presentacion_nombre']?.toString() ?? 'Unitario',
      conversionFactor: 1.0,
      prices: [
        PuntoPrecio(
            type: 'NORMAL',
            amount: (map['precio_unitario'] as num?)?.toDouble() ?? 0.0)
      ],
    );
    ...
    return ItemCarrito(
      ...
      manualPriceOverride: null, // El precio ya está capturado en precio_unitario
      manualPriceLocked: false,
