...
class AtributoPresentacion {
  final String key;
  final String value;

  AtributoPresentacion({required this.key, required this.value});

  factory AtributoPresentacion.fromJson(Map<String, dynamic> json) {
    return AtributoPresentacion(
      key: json['key'] ?? '',
      value: json['value'] ?? '',
    );
  }
...
class PuntoPrecio {
  ...
  factory PuntoPrecio.fromJson(Map<String, dynamic> json) {
    return PuntoPrecio(
...
class Presentacion {
  ...
  final List<PuntoPrecio> prices; // [normal, wholesale, special]
  final List<AtributoPresentacion> attributes; // [color, tamaño, etc]
  ...
  factory Presentacion.fromJson(Map<String, dynamic> json) {
    return Presentacion(
      ...
      prices: json['prices'] != null
          ? (json['prices'] as List).map((p) => PuntoPrecio.fromJson(p)).toList()
          : [],
      attributes: json['attributes'] != null
          ? (json['attributes'] as List)
              .map((a) => AtributoPresentacion.fromJson(a))
              .toList()
          : [],
  ...
  Presentacion copyWith({
    ...
    String? parentId,
    List<PuntoPrecio>? prices,
    List<AtributoPresentacion>? attributes,
    String? barcode,
    bool? isActive,
    DateTime? createdAt,
    DateTime? updatedAt,
  ...
  static Presentacion get unit => Presentacion(
    id: 'unit',
    skuCode: 'UNIT-001',
    name: 'UNIDAD',
  ...
  @override
  String toString() => 'Presentacion($skuCode: $name @ $conversionFactor x ${getPriceByType("NORMAL")})';
}
