...
import 'package:bipenc/servicios/servicio_seguridad.dart';
import 'package:image_picker/image_picker.dart';
import '../../servicios/servicio_pdf.dart';
import 'package:bipenc/servicios/servicio_identidad.dart';
...
class _PantallaAjustesState extends State<PantallaAjustes> {
  final _securityService = ServicioSeguridad();
  ...
  // API DNI
  final _dniUrlCtrl = TextEditingController();
  final _dniTokenCtrl = TextEditingController();
  ...
  Future<void> _loadBusinessData() async {
    ...
    _dniUrlCtrl.text = prefs.getString('dni_api_url') ?? '';
    _dniTokenCtrl.text = prefs.getString('dni_api_token') ?? '';
    setState(() {
      _soloWifi = prefs.getBool('sync_solo_wifi') ?? true;
      _modoPrueba = prefs.getBool('modo_prueba_flag') ?? false;
    });
  }
  ...
  Future<void> _guardarConfiguracion() async {
    if (_formKey.currentState!.validate()) {
      ...
      await prefs.setBool('modo_prueba_flag', _modoPrueba);
      ...
      await prefs.setString('dni_api_url', _dniUrlCtrl.text.trim());
      await prefs.setString('dni_api_token', _dniTokenCtrl.text.trim());
  ...
  Future<void> _previewPdf() async {
    ...
    try {
      ...
      final file = await ServicioPdf.generateVentaPdf(
        items: items,
      ...
      await ServicioPdf.shareVentaPdf(file);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _probarDniApi() async {
    final url = _dniUrlCtrl.text.trim();
    final token = _dniTokenCtrl.text.trim();
    if (url.isEmpty || token.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Configura URL y Token primero'),
            backgroundColor: Colors.orange),
      );
      return;
    }
    try {
      final res = await ServicioIdentidad.consultarDni('12345678');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(res != null
                  ? 'OK: ${res.nombreCompleto}'
                  : 'Sin respuesta / cache'),
              backgroundColor: res != null ? Colors.green : Colors.orange),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error consultando: $e'), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  List<ItemCarrito> _mockItems() {
    final pres = Presentacion(
      ...
      conversionFactor: 1,
      prices: [
        PuntoPrecio(type: 'NORMAL', amount: 5.5),
        PuntoPrecio(type: 'WHOLESALE', amount: 5.0),
        PuntoPrecio(type: 'SPECIAL', amount: 4.5),
      ],
    );
    final prod = Producto(
      id: 'prod1',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: 'Series y correlativos',
            icon: const Icon(Icons.confirmation_number_outlined),
            onPressed: () => context.push('/settings/correlativos'),
          ),
        ],
      ),
      body: DecoratedBox(
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const Divider(),
                      const SizedBox(height: 12),
                      const Text('API DNI (opcional)',
                          style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                      const SizedBox(height: 10),
                      TextFormField(
                        controller: _dniUrlCtrl,
                        decoration: const InputDecoration(
                          labelText: 'Endpoint API DNI',
                          border: OutlineInputBorder(),
                          helperText: 'Ej: https://api.midni.pe/dni',
                        ),
                      ),
                      const SizedBox(height: 10),
                      TextFormField(
                        controller: _dniTokenCtrl,
                        decoration: const InputDecoration(
                          labelText: 'Token Bearer',
                          border: OutlineInputBorder(),
                          helperText: 'Se guarda localmente (SharedPreferences)',
                        ),
                        obscureText: true,
                      ),
                      const SizedBox(height: 12),
                      OutlinedButton.icon(
                        onPressed: _probarDniApi,
                        icon: const Icon(Icons.search),
                        label: const Text('Probar consulta DNI (12345678)'),
                        style: OutlinedButton.styleFrom(
                          foregroundColor: Colors.tealAccent,
                          side: const BorderSide(color: Colors.tealAccent),
                        ),
