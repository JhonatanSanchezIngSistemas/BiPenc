...
class _PantallaPerfilState extends State<PantallaPerfil> {
  ...
  Future<void> _guardarPerfil() async {
    ...
    final ok = await ServicioBackend.actualizarPerfil(
  ...
  Future<void> _cambiarPassword() async {
    ...
    final err = await ServicioBackend.cambiarPassword(nuevaPassword: p1);
    if (!mounted) return;
  ...
  Future<void> _guardarEmail() async {
    ...
    final err = await ServicioBackend.cambiarEmail(nuevoEmail: email);
    if (!mounted) return;
  ...
  Future<void> _seleccionarFoto() async {
    ...
    final url = await ServicioBackend.subirAvatarPerfil(File(photo.path));
    if (!mounted) return;
    if (url == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
