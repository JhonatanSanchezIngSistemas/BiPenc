...
import '../caja/proveedor_caja.dart';
import '../../servicios/servicio_despacho.dart';
import '../../utilidades/fiscal.dart';

class PantallaPanelConfiguracion extends StatelessWidget {
  const PantallaPanelConfiguracion({super.key});
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    if (isAdmin) ...[
                      _buildSectionHeader('Panel Súper-Admin'),
                      const SizedBox(height: 12),
                      _buildActionCard(
                        context,
                        title: 'Monitoreo y Auditoría',
                        subtitle: 'Métricas de ingresos, logs de seguridad y anulaciones',
            ...
            children: [
              Text('Consola Administrativa',
                  style: TextStyle(
                      ...
                      fontSize: 20,
                      fontWeight: FontWeight.bold)),
              Text('Panel de operación del negocio',
  ...
  Widget _buildStatusRow() {
    return StreamBuilder<bool>(
        stream: ServicioImpresion().connectionStream,
        initialData: ServicioImpresion().estaConectado,
  ...
  Widget _buildPedidosRecientes(BuildContext context) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      future: ServicioDbLocal.getUltimasVentas(limit: 5),
      builder: (context, snapshot) {
          ...
          children: snapshot.data!.map((v) {
            ...
            return Card(
              ...
              child: ListTile(
                ...
                title: Text('Ticket ${v['correlativo'] ?? v['id']}',
                ...
                onTap: () async {
                  ...
                  final venta = Venta(
                    ...
                    tipoComprobante:
                        (v['correlativo']?.toString().contains('F') ?? false)
                    ...
                    metodoPago: MetodoPago.values.firstWhere(
                      (e) =>
                          e.name.toUpperCase() ==
                          (v['metodo_pago']?.toString().toUpperCase() ??
                              'EFECTIVO'),
                    ...
                    items: (itemsRaw as List)
                        .map((i) =>
                            ItemCarrito.fromJson(i as Map<String, dynamic>))
                        .cast<ItemCarrito>()
                        .toList(),
                    dbId: v['id'] as int,
                  ...
                  if (context.mounted) {
                    await ServicioDespacho().abrirPicking(context, venta);
                  }
                },
              ),
            );
  ...
  Widget _buildProfileCard(BuildContext context) {
    return FutureBuilder(
      future: ServicioSesion().obtenerPerfil(forceRefresh: true),
      builder: (context, snapshot) {
        final perfil = snapshot.data;
        final nombre = perfil?.nombreCompleto.trim().isNotEmpty == true
            ? perfil!.nombreCompleto
            : 'Usuario';
        final alias = perfil?.alias.trim().isNotEmpty == true
            ? perfil!.alias
            : 'SIN ALIAS';
          ...
          child: ListTile(
            ...
            title: Text(nombre,
                style: const TextStyle(
                    ...
                    fontWeight: FontWeight.bold,
                    fontSize: 16)),
            subtitle: Text('Alias: $alias  •  Rol: $rol',
