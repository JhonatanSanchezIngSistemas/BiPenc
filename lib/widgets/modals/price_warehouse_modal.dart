...
import '../../data/models/presentation.dart';
import '../common/robust_image.dart';

class PriceWarehouseModal extends StatelessWidget {
  final Producto producto;
  final Function(Presentacion) onPriceSelected;
  final bool isAdmin;

  const PriceWarehouseModal({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    final pUnit = producto.presentaciones.firstWhere(
      (p) => p.id == 'unid',
      orElse: () => Presentacion(id: 'unid', skuCode: producto.skuCode, name: 'Unitario', conversionFactor: 1, prices: [PricePoint(type: 'NORMAL', amount: producto.precioBase)]),
    );
    
    final pMayo = producto.presentaciones.firstWhere(
      (p) => p.id == 'mayo',
      orElse: () => Presentacion(id: 'mayo', skuCode: producto.skuCode, name: 'Mayorista', conversionFactor: 1, prices: [PricePoint(type: 'NORMAL', amount: producto.precioMayorista)]),
    );

    final pEspe = producto.presentaciones.firstWhere(
      (p) => p.id == 'espe',
      orElse: () => Presentacion(id: 'espe', skuCode: producto.skuCode, name: 'Especial', conversionFactor: 1, prices: [PricePoint(type: 'NORMAL', amount: producto.precioEspecial)]),
    );
