...
void main() {
  group('ServicioSupabase Compatibility', () {
    test('remueve campo despachado cuando columna no existe', () {
      ...
      final next = ServicioSupabase.nextVentaPayloadForCompatibilityForTest(payload, e);
    ...
    test('fallback correlativo genera prefijo temporal', () {
      final corr = ServicioSupabase.generarCorrelativoFallbackForTest();
      expect(corr.startsWith('T-'), isTrue);
      expect(corr.length, greaterThan(6));
    });
  });

  group('ServicioSupabase mapToProducto', () {
    test('usa fallback de presentaciones cuando viene sin presentaciones', () {
      final p = ServicioSupabase.mapToProducto({
        'id': 'abc',
        'sku': 'SKU-01',
        'nombre': 'Producto',
        'marca': 'Marca',
