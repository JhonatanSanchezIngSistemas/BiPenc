...
void main() {
  ...
  setUp(() async {
    ...
    PrintQueue.databaseOverride = () async => db;
    PrintQueue.perItemDelayOverride = () => Duration.zero;
  });

  tearDown(() async {
    PrintQueue.resetTestingOverrides();
    await db.close();
  });

  test('encolarImpresion guarda pendiente cuando no hay conexión', () async {
    PrintQueue.connectionStatusOverride = () async => false;
    var writes = 0;
    PrintQueue.writeBytesOverride = (_) async {
      writes++;
      return true;
    };
  ...
  test('procesarCola marca como impreso cuando writeBytes retorna true',
    ...
    PrintQueue.connectionStatusOverride = () async => true;
    PrintQueue.writeBytesOverride = (_) async => true;
  ...
  test('procesarCola incrementa intentos cuando writeBytes falla', () async {
    ...
    PrintQueue.connectionStatusOverride = () async => true;
    PrintQueue.writeBytesOverride = (_) async => false;
  ...
  test('procesarCola registra error_msg truncado cuando hay excepción',
    ...
    PrintQueue.connectionStatusOverride = () async => true;
    PrintQueue.writeBytesOverride = (_) async => throw Exception('x' * 160);
